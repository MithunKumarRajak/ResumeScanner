...
export default function ExperienceTimeline({ workHistory = [], totalYears = 0, careerGaps = [] }) {
  ...
  const recentRoles = workHistory.slice(0, 2)

  return (
    <div className="glass-card p-6 animate-slide-up">
      <div className="flex items-center gap-2 mb-4 pb-4 border-b border-slate-700/50">
        <Briefcase className="h-5 w-5 text-indigo-400" />
        <h3 className="font-semibold text-white">Experience Snapshot</h3>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-5">
        <div className="rounded-xl border border-slate-700/50 bg-slate-900/30 px-4 py-3">
          <p className="text-[11px] uppercase tracking-[0.2em] text-slate-500">Roles extracted</p>
          <p className="mt-1 text-lg font-semibold text-white">{workHistory.length}</p>
        </div>
        <div className="rounded-xl border border-slate-700/50 bg-slate-900/30 px-4 py-3">
          <p className="text-[11px] uppercase tracking-[0.2em] text-slate-500">Current focus</p>
          <p className="mt-1 text-lg font-semibold text-white">{recentRoles[0]?.title || 'Inferred role'}</p>
        </div>
        <div className="rounded-xl border border-slate-700/50 bg-slate-900/30 px-4 py-3">
          <p className="text-[11px] uppercase tracking-[0.2em] text-slate-500">Timeline quality</p>
          <p className="mt-1 text-lg font-semibold text-white">Structured</p>
        </div>
      </div>

      <details>
        <summary className="flex cursor-pointer list-none items-center justify-between gap-3 rounded-xl border border-slate-700/50 bg-slate-900/20 px-4 py-3 text-sm font-medium text-slate-200">
          <span>View extracted timeline</span>
          <span className="text-xs text-slate-500">optional details</span>
        </summary>

        <div className="mt-5 relative pl-3 space-y-6 before:absolute before:inset-0 before:ml-5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-slate-700 before:to-transparent">
          {timelineItems.map((item, idx) => {
            if (item.type === 'job') {
              const title = item.title || 'Role inferred from resume'
              return (
                <div key={`job-${idx}`} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
                  ...
                  <div className="w-[calc(100%-2.5rem)] md:w-[calc(50%-1.5rem)] p-4 rounded-xl border border-slate-700/50 bg-slate-800/30 backdrop-blur-sm shadow-xl transition-all hover:bg-slate-800/50 hover:border-indigo-500/30">
                    <div className="flex flex-col gap-1">
                      ...
                      <p className="text-sm font-medium text-indigo-400">{item.company || 'Company not specified'}</p>
