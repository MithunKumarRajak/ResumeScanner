...
class MultilingualPreprocessor:
    def __init__(self):
        ...
        try:
            ...
            self.nlp_xx = None
            print("WARNING: xx_ent_wiki_sm not found. Run: python -m spacy download xx_ent_wiki_sm")
    ...
    def clean_text(self, text: str, lang: str = 'en') -> str:
        ...
        if lang == 'hi':
            ...
            text = re.sub(r'[^\u0900-\u097F\u0041-\u007A\u0061-\u007A\s\d]', ' ', text)
        else:
...
class BiasDetector:
    ...
    NAME_PATTERNS_SOUTH_ASIAN = ['kumar', 'singh', 'sharma', 'patel', 'rajak', 'verma', 'gupta', 'rao', 'reddy']
...
class SemanticMatcher:
    ...
    def match_resume_to_jd(self, resume_text: str, jd_text: str) -> dict:
        ...
        STOP = {'the','a','an','and','or','in','of','to','for','is','are','with','on','at','by','be','this','that','as','from'}
        missing_keywords = [w for w in jd_words - resume_words if len(w) > 3 and w not in STOP]
        
        return {
            'semantic_score': score,
            'keyword_overlap_score': keyword_score,
            'combined_score': round(score * 0.7 + keyword_score * 0.3, 2),  # weighted
...
class XAIExplainer:
    ...
    def _build_explainer(self, X_background):
        ...
        self.explainer = shap.LinearExplainer(self.model, background, feature_perturbation='interventional')
    ...
    def explain_resume(self, resume_text: str, preprocessed_text: str, 
        ...
        feature_vec = np.hstack([tfidf_vec, extra_features])  # shape (1, n_features)
        ...
        class_shap = shap_values[pred_class_idx][0] if isinstance(shap_values, list) else shap_values[0]
    ...
    def _generate_summary(self, category: str, confidence: float, top_features: list) -> str:
        ...
        conf_label = 'highly confident' if confidence > 0.8 else ('moderately confident' if confidence > 0.6 else 'low confidence')
        return (
...
def merge_small_classes(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for small_cat, parent_cat in merge_rules.items():
        if small_cat in df['Category'].values:
            ...
            if sample_count < 10:
                print(f'  Merging {small_cat} ({sample_count} samples) -> {parent_cat}')
...
def extract_features_v6(text: str, lang: str = 'en') -> dict:
    ...
    # Years experience
    yexp = re.search(r'(\d+)\+?\s*years?\s*(of\s*)?(experience|exp)', text_lower)
    if yexp:
    ...
    features['is_technical'] = min(sum(1 for k in tech_kw if k in text_lower) / 5.0, 1.0)
    ...
    # Certifications
    cert_matches = re.findall(r'\b(certified|certification|certificate|aws|gcp|azure|pmp|cfa|cissp|comptia)\b', text_lower)
    features['certification_count'] = min(len(cert_matches) / 5.0, 1.0)
    
    # Skill density
    words = text_lower.split()
    skill_count = sum(1 for k in tech_kw if k in text_lower)
    features['skill_density'] = min(skill_count / max(len(words), 1) * 100, 1.0)
    
    # Seniority
    senior_kw = ['senior', 'lead', 'principal', 'director', 'vp', 'chief', 'head']
    ...
    features['action_verb_count'] = min(sum(1 for v in action_verbs if v in text_lower) / 6.0, 1.0)
    
    # Quantified achievements
    quant_matches = re.findall(r'\d+\s*(%|percent|people|team|million|thousand|k\b)', text_lower)
...
def fine_tune_on_custom_data(
    ...
    custom_df['Processed'] = custom_df['Resume'].apply(lambda t: preprocessor.preprocess(t)['processed'])
    ...
    X_custom_tfidf = tfidf.transform(custom_df['Processed'])
    
    # Partial fit (warm start) - works with SGDClassifier
    base_sgd = model.estimator if hasattr(model, 'estimator') else model
    
    print(f"  Fine-tuning for {epochs} epochs on {len(custom_df)} samples...")
    for epoch in range(epochs):
        base_sgd.partial_fit(X_custom_tfidf, y_custom, classes=np.arange(len(le.classes_)))
        y_pred = base_sgd.predict(X_custom_tfidf)
...
def train_v6(args):
    ...
    print("=" * 80)
    print("ResumeModel V6 - Full Phase 3 Training")
    ...
    features_list = [extract_features_v6(row['Resume'], row['lang']) for _, row in df.iterrows()]
    ...
    pre_bias = bias_detector.generate_bias_report(df, df['Category'].tolist(), 'lang')
    ...
    embeddings = matcher.model.encode(all_texts, show_progress_bar=True, normalize_embeddings=True)
    X_transformer = np.array(embeddings)

    # [7] Build combined feature matrix
    print("\n[7/9] Building feature matrix...")
    tfidf = TfidfVectorizer(max_features=15000, sublinear_tf=True, ngram_range=(1, 3))
    ...
    try:
        ...
        dpd = demographic_parity_difference(y_test, y_pred, sensitive_features=test_lang_list)
        post_bias_report = {'demographic_parity_difference': round(float(dpd), 4)}
    except Exception as e:
    ...
    with open(out_dir / 'bias_report.json', 'w') as f:
        json.dump({'pre_training': pre_bias, 'post_training': post_bias_report}, f, indent=2)
    ...
    manifest = {
        ...
        'phase3_features': ['semantic_matching', 'bias_detection', 'xai_shap', 'multilingual', 'custom_training'],
...
if __name__ == '__main__':
    p = argparse.ArgumentParser(description='Train ResumeModel V6 — Phase 3 Full')
    p.add_argument('--data-dir', default='./Dataset', help='Dataset directory')
    p.add_argument('--out-dir', default='./models/v6', help='Output directory for artifacts')
    p.add_argument('--embedder', default='paraphrase-multilingual-MiniLM-L12-v2',
                   help='sentence-transformers model name')
    p.add_argument('--fine-tune', action='store_true', help='Fine-tune existing model instead of training from scratch')
    p.add_argument('--fine-tune-data', default=None, help='Path to company CSV for fine-tuning')
    p.add_argument('--fine-tune-base', default='./models/v6', help='Base model dir for fine-tuning')
    p.add_argument('--fine-tune-out', default='./models/v6-custom', help='Output dir for fine-tuned model')
    args = p.parse_args()
    
    if args.fine_tune:
        assert args.fine_tune_data, "--fine-tune-data required with --fine-tune"
        fine_tune_on_custom_data(args.fine_tune_base, args.fine_tune_data, args.fine_tune_out)
    else:
        train_v6(args)
